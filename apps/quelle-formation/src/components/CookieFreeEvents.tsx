...
type EventType = "page_view" | "vendor_interest" | "vendor_outbound" | "contact_click" | "lead_start" | "lead_submit";
...
export function CookieFreeEvents() {
  ...
  useEffect(() => {
    ...
    function onFocus(event: FocusEvent) {
      ...
      if (!form || form.dataset.usineEvents !== "lead" || startedForms.has(form)) return;
      startedForms.add(form);
      send("lead_start", form.dataset.usineTarget, form.dataset.usinePlacement ?? form.id ?? "form");
    }

    function onSubmit(event: SubmitEvent) {
      const form = event.target instanceof HTMLFormElement ? event.target : null;
      if (!form || form.dataset.usineEvents !== "lead") return;
      send("lead_submit", form.dataset.usineTarget, form.dataset.usinePlacement ?? form.id ?? "form");
    }
